...
- After raw FastQC, the class-style cleanup step is trimming + re-QC. Use `fastp` (preferred) or **FASTX trimming** (e.g., `fastq_quality_trimmer` and/or `fastx_clipper`) followed by re-running FastQC on the trimmed reads.
